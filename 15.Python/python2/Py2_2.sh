...
● 匿名函数
lambda优点
	lambda能和def做同样种类的工作，特别是对于那些逻辑简单的函数，
	直接用lambda会更简洁，
...
● 偏函数  #partial
  — 相当于改造现有函数，将现有函数的一部分参数固定下来
	...
	112	
##############################################################################
非重点● tkinter模块、偏函数  #脚本请看  tkinter_bibao.py
...
def hello(word):
    def say_hi():
        lb.config(text=word)
    return say_hi         #没有加(),是引用函数

##############################################################################
装饰器  
— 当需要给某个模块添加功能，又不便于修改源代码时，可以使用装饰器
	#!/usr/bin/env python3
	def set_color(func):
		def color():
	...
	if __name__ == '__main__':
		hh=set_color(hello)
		print(hh())
	   print(hello())  #因为 @set_color 调用的时set_color(greet).color()
	   print(greet())  #因为 @set_color 调用的时set_color(greet).color()

	#运行结果：
	Hello World!  #红色的
	<function set_color.<locals>.color at 0x7f2cbfd06378>  #闭包函数本身
	How are you?  #红色的
