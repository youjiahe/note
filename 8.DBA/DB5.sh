问题：
1.LSN日志序列号与执行的命令有什么联系？
2.日志跟库文件（frm,ibd）有什么联系？
3.为什么一定要恢复日志文件，才能恢复数据？
4.恢复时，直接拷贝备份目录下某个库下的文件到数据库目录不就可以了吗？
5.为了更好理解，验证事务特性，能不能在明确地给一些例子在笔记上？
6.主从同步中，客户端在主库上做删除操作，从库也会同步吗？
##################################################################################
mysqldump备份/恢复
●数据备份方式：
  &物理备份
...
●初始化数据库
  ...
  grep “t.* password“ /var/log/mysqld.log | less  #                                                                        " /var/log/mysql 
  mysql -uroot -p初始密码
##################################################################################  
逻辑备份方式 
●完全备份
